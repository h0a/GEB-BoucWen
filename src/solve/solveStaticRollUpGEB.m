function [beam, mesh] = solveStaticRollUpGEB(beam,mesh)
    % Solving statics problem of a planar rollup cantilever using a 3-fields mixed formulation 
    ...
    % external force vector (config-independent forces)
    ...
    for i = 1:beam.numLoadSteps
        ...
        NRiter = 0;
         ...
         while NRiter < beam.maxNumIter
            ...
            [x, deltax] = solveLinSysStaticRollUpGEB(beam,mesh,alpha,x);
...
function [x, deltax] = solveLinSysStaticRollUpGEB(beam,mesh,loadFactor,x)
    % Solving the linearized system of equations of a planar rollup cantilever using a 3-fields mixed formulation 
    % and Lagrange multiplier to enforce orthonormal nodal director constraint and the Newton-Raphson scheme.
