function r = elementBWConstitutiveResidual(beam, mesh, eleId, x, z)
...
%         z:         fixed value of z for each strain dof when using the Bouc-Wen model
% output: r:         constitutive residual vector 6x1 (6 strain/stress components per element)


% (element) e, s, and z of the current element
ee = x(mesh.LMstrains(:,eleId));
se = x(mesh.LMstresses(:,eleId));
ze = z(mesh.LMstrains(:,eleId) - mesh.ndofs);
...
sigmaBW = 0.5 .* beam.eleSigmaFunc(ee,ze) + 0.5 .* (beam.AmatSigma * beam.elasMatrix * ee);
r = (sigmaBW - se) .* sum(w .* J1 .* J2);